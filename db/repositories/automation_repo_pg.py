"""Supabase Postgres repository for tenant-scoped CRM automations (Phase 3).

Drop-in equivalent of ``db.repositories.automation_repo`` backed by Postgres.
"""
...
def _record_run(
    ...
    pg.execute(
        """
        INSERT INTO automation_runs (tenant_slug, rule_id, deal_id, fingerprint, trigger_type, status, context, result, error, ts)
...
def create_rule(data: dict) -> dict:
    ...
    row = pg.execute_returning(
        ...
        INSERT INTO automation_rules (
            tenant_slug, name, enabled, trigger_type, from_stage, to_stage,
            condition_owner, condition_min_value, condition_tag,
...
def update_rule(rule_id: int, data: dict) -> dict | None:
    ...
    pg.execute(
        ...
        SET name = %s, enabled = %s, trigger_type = %s, from_stage = %s, to_stage = %s, condition_owner = %s, condition_min_value = %s, condition_tag = %s, action_type = %s, action_payload = %s, updated_at = %s
        WHERE id = %s AND tenant_slug = %s
        """,
        (
            payload["name"],
...
def delete_rule(rule_id: int) -> bool:
    slug = pg._get_slug()
    rows = pg.execute("DELETE FROM automation_rules WHERE id = %s AND tenant_slug = %s", (rule_id, slug))
    return rows > 0
...
def apply_deal_stage_changed(before: dict, after: dict) -> int:
    ...
    rows = pg.fetchall(
        ...
        SELECT id, name, enabled, trigger_type, from_stage, to_stage, action_type, action_payload, created_at, updated_at
             , condition_owner, condition_min_value, condition_tag
        FROM automation_rules
        WHERE trigger_type = 'deal_stage_changed' AND enabled = 1 AND tenant_slug = %s
        ORDER BY id ASC
        """,
        (slug,)
    )
    ...
    for row in rows:
        ...
        try:
            if rule["action_type"] == "create_task":
                ...
                # UPDATE DEALS in Postgres directly using tenant connection
                pg.execute(
                    "UPDATE crm_deals SET stage = %s, updated_at = %s WHERE id = %s AND tenant_slug = %s",
                    (next_stage, time.time(), int(after["id"]), slug)
                )
...
def _was_fingerprint_recent(fingerprint: str, window_seconds: float) -> bool:
    ...
    row = pg.fetchone(
        ...
        LIMIT 1
        """,
        (str(fingerprint or ""), slug, cutoff),
    )
